        ...
        with dask_client.as_current():
            ...
            for strategy in ["natural", "compact", "split"]:
                ...
                for worker, stats in worker_summary.iterrows():
                    logger.info(
                        f"  {worker}: {stats['count']} partitions, "
                        f"{stats['sum']} total rows"
                    )
        ...
        with dask_client.as_current():
            ...
            def show_connection_info(partition):
                ...
                return pd.DataFrame({
                    ...
                    'connection_info': [f"Would connect to Cassandra from {socket.gethostname()}"],
                    'query_info': [f"Would execute: SELECT * FROM table WHERE token(pk) > X AND token(pk) <= Y"],
